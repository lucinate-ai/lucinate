...
// assertNoChatEvent fails if any chat event surfaces within d.
func assertNoChatEvent(t *testing.T, b *Backend, d time.Duration) {
	t.Helper()
	...
	for {
		...
		case ev := <-b.Events():
			if ev.EventName == protocol.EventChat {
				t.Fatalf("unexpected chat event: %s", ev.Payload)
			}
		case <-deadline:
			return
		}
...
// A v2026.7.x gateway cancels a turn interrupted during agent
// initialisation without emitting any terminal frame. The fallback
// timer synthesises the aborted event, late frames for the killed run
// are absorbed, and the session stays usable.
func TestChatAbort_FallbackWhenGatewayStaysSilent(t *testing.T) {
	restore := abortFallbackGrace
	abortFallbackGrace = 50 * time.Millisecond
	defer func() { abortFallbackGrace = restore }()

	g := newFakeGateway(t, "tok")
	onCreateSession(g)
	// The silent-kill shape: prompt.submit is accepted but no events
	// ever follow; session.interrupt is acknowledged, also eventless.
	g.on("prompt.submit", func(id uint64, _ json.RawMessage) {
		g.respond(id, map[string]any{"status": "streaming"})
	})
	g.on("session.interrupt", func(id uint64, _ json.RawMessage) {
		g.respond(id, map[string]any{"status": "interrupted"})
	})
	b := newTestBackend(t, g, "tok")
	connectBackend(t, b)
	...
	res, err := b.ChatSend(context.Background(), key, backend.ChatSendParams{Message: "long essay"})
	if err != nil {
		t.Fatalf("ChatSend: %v", err)
	}
	if err := b.ChatAbort(context.Background(), key, res.RunID); err != nil {
		t.Fatalf("ChatAbort: %v", err)
	}

	aborted := waitChatEvent(t, b, "aborted")
	if aborted.RunID != res.RunID || aborted.SessionKey != "live1" {
		t.Fatalf("synthesised abort envelope = %+v", aborted)
	}

	// A late terminal frame for the killed run is absorbed.
	g.push(map[string]any{"type": "message.complete", "session_id": "live1",
		"payload": map[string]any{"text": "late tail", "status": "interrupted"}})
	assertNoChatEvent(t, b, 150*time.Millisecond)

	// The session takes the next turn normally.
	g.on("prompt.submit", func(id uint64, _ json.RawMessage) {
		g.respond(id, map[string]any{"status": "streaming"})
		g.push(map[string]any{"type": "message.complete", "session_id": "live1",
			"payload": map[string]any{"text": "ok", "status": "complete"}})
	})
	res2, err := b.ChatSend(context.Background(), key, backend.ChatSendParams{Message: "again"})
	if err != nil {
		t.Fatalf("ChatSend after abort: %v", err)
	}
	final := waitChatEvent(t, b, "final")
	if final.RunID != res2.RunID {
		t.Fatalf("final run id = %q, want %q", final.RunID, res2.RunID)
	}
}

// An abort acknowledged after the turn already finalised must not
// synthesise a spurious aborted event once the grace elapses.
func TestChatAbort_NoSpuriousAbortAfterFinal(t *testing.T) {
	restore := abortFallbackGrace
	abortFallbackGrace = 50 * time.Millisecond
	defer func() { abortFallbackGrace = restore }()
	...
	g.on("prompt.submit", func(id uint64, _ json.RawMessage) {
		g.respond(id, map[string]any{"status": "streaming"})
		g.push(map[string]any{"type": "message.complete", "session_id": "live1",
			"payload": map[string]any{"text": "done", "status": "complete"}})
	})
	g.on("session.interrupt", func(id uint64, _ json.RawMessage) {
		g.respond(id, map[string]any{"status": "interrupted"})
	})
	...
	if _, err := b.ChatSend(context.Background(), key, backend.ChatSendParams{Message: "hi"}); err != nil {
		t.Fatalf("ChatSend: %v", err)
	}
	waitChatEvent(t, b, "final")

	if err := b.ChatAbort(context.Background(), key, ""); err != nil {
		t.Fatalf("ChatAbort: %v", err)
	}
	assertNoChatEvent(t, b, 200*time.Millisecond)
}
